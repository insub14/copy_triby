...
<%@ include file='../common/footer.jsp' %>
    ...
    $(function(){
    	...
    	$(document).on("click","#writeReview",function(e){
    		...
    		var hNo=$(".host>a").attr("href").replace("detail.ho?tag=트리비&hNo=","");//링크작업후 replace할것
    		...
    		success:function(data){
    			...
    			case "myTribyPre":
    				...
    				$.each(data, function(i,v){
    					...
    					var $myTribyTitle=$("<div class='myTribyTitle'>").append($("<h4>").text(v.title)).append($("<h5>").text(v.address)).append($("<div class='host'>").append($("<a>").attr("href","detail.ho?tag=트리비&hNo="+v.hNo).text(v.hostName)));
    			...
    			case "myReview":
    				...
    				$.each(data, function(i,v){
    					...
    					if(v.reply!=null){
    						...
    						var $myReviewTop2=$("<div class='myReviewTop'>").append($("<a>").attr("href","detail.ho?tag=트리비&hNo="+v.hNo).text(v.hostName));
    ...
    function writeAreviewForm(pNo){
    	...
    	$.ajax({
    		...
    		success:function(data){
				...
				var $myTribyTitle=$("<div class='myTribyTitle'>").append($("<h4>").text(data.title)).append($("<h5>").text(data.address)).append($("<div class='host'>").append($("<a>").attr("href","detail.ho?tag=트리비&hNo="+data.hNo).text(data.hostName)));
				$myTribyInfoWrap.append($myTribyThumb).append($myTribyTitle);
